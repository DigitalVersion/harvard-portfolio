...
/** Renders the resume viewer for a given `cvType`, defaulting to "default".
 * Used by both the root `/` route (query-param driven, kept for backward
 * compatibility with existing `?cvType=` links) and the path-based
 * `/[cvType]` route (one clean URL per job application variant). */
export default function ResumeApp({ initialCvType }: { initialCvType?: string }) {
  const router = useRouter();

  const cvType = initialCvType && resumes[initialCvType] ? initialCvType : "default";
  ...
  const handleTypeChange = (typeId: string) => {
    router.push(typeId === "default" ? "/" : `/${typeId}`);
  };
